...
public class r_robin_test{
    ...
    public static void main(String[] args){ 
         ...
         * TEST CASES
         * 
         * I have created 10 different combinations of :-
         * the number of processes for each test
         * the time quantum for each test
         * the arrival times for each process in each test
         * the burst times for each process in each test
         */
        int[] processCombinations = {3,3,3,3,4,4,4,5,5,5};
        int[] timeQuantumCombinations = {3,4,3,4,5,4,3,4,3,2};
        int[][] arrivalTimes = {{1,2,3}, {2,3,4}, {3,4,5}, {4,5,6}, {1,2,3,4}, {2,3,4,5}, {3,4,5,6}, {1,2,3,4,5}, {1,3,4,5,6}, {3,6,7,8,9}};
        int[][] burstTimes = {{4,2,5}, {3,6,4}, {6,5,2}, {1,6,3}, {3,6,4,2}, {7,3,8,5}, {9,6,3,6}, {2,3,4,5,1}, {3,4,3,4,5}, {2,3,4,6,6}};



        for(int t = 0; t < 10; t++) // running 10 tests
            {
        ...
        tQuantum = timeQuantumCombinations[t];
        n = processCombinations[t];
        ...
        for(int i = 0; i < n; i++)
            arrival[i] = arrivalTimes[t][i];  
 
        for(int i = 0; i < n; i++){
            burst[i] = burstTimes[t][i];   
            remBurst[i] = burst[i];   
        }
        ...
        String fs = String.format("%.02f", avgWait/n);

        System.out.print("\nAverage wait time for test [" + (t+1) + "] with " + n + " processes and time quantum of " + tQuantum+ ": " + fs + "ms \n"); // print out the average wating time of the scheduler
